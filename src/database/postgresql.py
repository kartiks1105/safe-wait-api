...
class PostgreSQL:
    ...
    def getStudentInformation(self, student_id, password):
        query = """
    ...
    def getDriverInformation(self, student_id, password):
        query = """
            SELECT * FROM public.accounts
            WHERE student_id = %s
            AND password = %s
            AND profile_type = %s
        """
        self.cursor.execute(query, [student_id, password, "Driver"])
        row = self.cursor.fetchone()
        if row:
            account_data = {
                'student_id': row[0],
